...
import { AbsenceTypeRowType } from 'src/types/apps/absenceTypes'

// ** Actions Imports
import { getTypeAbsence, deleteTypeAbsence, UpdateTypeAbsence } from 'src/store/settings/configAbsence'
...
const TypeAbsenceTable = () => {
  ...
  const updateTypeAbsenceMutation = useMutation(UpdateTypeAbsence, {
    onSuccess: () => {
      queryClient.invalidateQueries('typeAbsence')
      toast.success('updated successfully')
    }
  })
  const handleEditTypeAbsence = (id: any, name: string, color: string) => {
    setName(name)
    setIdType(id)
    setColor(color)
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditTypeAbsence(row.id, row.name, row.color)}>
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Card>
            ...
            <CardHeader title='Types dabsence' style={{ textAlign: 'center', marginTop: '-20px' }}></CardHeader>
            {
              <CardContent style={{ textAlign: 'center', marginTop: '-10px' }}>
                Gérez les types d'absences au sein de votre entreprise
...
export default TypeAbsenceTable
