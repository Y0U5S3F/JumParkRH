...
import TableHeader from 'src/views/apps/departments/TableHeader'
...
const DepatementsTable = () => {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    updatedepartmentMutation.mutate({ iddepartment, data })
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditdepartment(row.id, row.name)}>
  ...
  return (
    <>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <PageHeader title={<Typography variant='h5'>departments List</Typography>} />
        </Grid>
        <Grid item xs={12}>
          <Card>
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <Icon icon='mdi:store-marker-outline' color='#915592' width='60' height='80' />
            </div>
            <CardHeader title='department List' style={{ textAlign: 'center', marginTop: '-20px' }}></CardHeader>
